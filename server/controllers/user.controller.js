...
export default {
  registerUser: (req, res) => {
    try {
      ...
      query(sql, [nickname]).then(async (rows) => {
        if (rows.length)
          return res.json({
            status: 401,
            msg: "Su usuario ya se encuentra ocupado",
          });
        ...
        const sql =
          "INSERT INTO UserData (nickname, password, pregunta, respuesta) values(?, ?, ?, ?)";
        const result = await query(sql, parametros);
        res.json({
          status: 200,
          msg: "Se ha registrado correctamente su usuario",
        });
      });
    } catch (e) {
      return res.json({
